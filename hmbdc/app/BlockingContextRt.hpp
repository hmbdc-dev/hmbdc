...
namespace blocking_context_rt_detail {
...
struct TypeSafeCopyOut {
    void* operator()(void*, void*, size_t) const {
        HMBDC_THROW(std::runtime_error, "not expected bytes");
    }
};

template <MessageC Message, MessageC ...Ms>
struct TypeSafeCopyOut<std::tuple<Message, Ms...>> {
    void* operator()(void* dest, void* bufferBytes, size_t size) const {
        if constexpr (Message::justBytes) {
            return memcpy(dest, bufferBytes, size);
        } else {
            auto& head = *reinterpret_cast<MessageHead*>(bufferBytes);
            if (typeTagMatch<Message>((uint16_t)head.typeTag)) {
                using Wrap = MessageWrap<Message>;
                auto& wrap = static_cast<Wrap&>(head);
                new (dest) Wrap(wrap);
                wrap.~Wrap();
                return dest;
            }
            return TypeSafeCopyOut<std::tuple<Ms...>>{}(dest, bufferBytes, size);
        }
    }
};
...
bool runOnceImpl(std::atomic<bool>& stopped
    , pattern::BlockingBufferRt* HMBDC_RESTRICT buf
    , CcClient& HMBDC_RESTRICT c, time::Duration maxBlockingTime) {
    pattern::BlockingBufferRt::iterator begin, end;
    try {
        if constexpr(std::is_base_of<time::TimerManagerTrait, CcClient>::value) {
        ...
        if (clientParticipateInMessaging && buf) {
            uint8_t msgSpace[buf->maxItemSize()] __attribute__ ((aligned));
            auto count = 0ul;
            if (buf->tryTake(TypeSafeCopyOut<typename std::decay_t<CcClient>::Interests>{}
                , msgSpace, 0, d)) {
                auto& msgHead = *reinterpret_cast<MessageHead*>(msgSpace);
                if (c.CcClient::handleImpl(msgHead, 0xffff)) count++;
            }
            c.CcClient::invokedCb(count ? 1 : 0);
        } else {
...
struct Transport {
    ...
    pattern::BlockingBufferRt buffer;
};
...
} //blocking_context_rt_detail

/**
 * @class BlockingContextRt<>
 * @brief A BlockingContextRt is similar to BlockingContext with the following diff
 * - When a Clinet's msg queue is full, new msg arriving would NOT block the sender, instead it pushes 
 * the oldest message out from the queue and they are intentionally 'lost'. This is desired in real time
 * applications that only need the latest messages and do not ever want the sender to be blocked
 * @details a Client running in such a BlockingContextRt utilizing OS's blocking mechanism
...
class BlockingContextRt {
private:
    using cpa = blocking_context_rt_detail::context_property_aggregator<MessageTuples ...>;
...
public:
    ...
    BlockingContextRt(BlockingContextRt const&) = delete;
    BlockingContextRt& operator = (BlockingContextRt const&) = delete;
     ...
     * @endcode
     * 
     * @param c Client - Regilar Client, cannot be RCLient of ClientWithStash
    ...
    void start(Client& c
        ...
        auto thrd = kickOffClientThread(blocking_context_rt_detail::runOnceImpl<Client>
    ...
    private: 
    using Transport = blocking_context_rt_detail::Transport;
    /**
     * @brief an ignore all data Client
     * 
     */
    struct IgnorePushedOut : client_using_tuple<IgnorePushedOut, Interests> {
        MessageDispacher<const IgnorePushedOut, Interests> disp;
        template<MessageC M> void handleMessageCb(M const&) const{}
        void handleJustBytesCb(uint16_t, uint8_t const*, app::hasMemoryAttachment*) const{}
        void operator()(void* pushedOutBytes) const {
            disp(*this, *reinterpret_cast<MessageHead*>(pushedOutBytes));
        }
    };
...
public:
    ...
    ClientRegisterHandle registerToRun(Client& c
        ...
        using RI = typename
            blocking_context_rt_detail::RealInterests<typename Client::Interests>::type;
    ...
    /**
     * @brief send a message to the BlockingContextRt to dispatch
    ...
    void send(Message&& m) {
        if constexpr (can_handle<Message>::match) {
            ...
            for (auto i = 0u; i < entries.size(); ++i) {
                auto& e = entries[i];
                if (e.deliverPred(m)) {
                    if constexpr (blocking_context_rt_detail::has_hmbdc_ctx_queued_ts<M>::value) {
                    ...
                    if (i == entries.size() - 1) {
                        e.transport->buffer
                            .template putInPlace<MessageWrap<M>>(IgnorePushedOut{}, std::forward<Message>(m));
                    } else {
                        e.transport->buffer.template putInPlace<MessageWrap<M>>(IgnorePushedOut{}, m);
                    }
                }
            }
        } else {
    ...
    void sendInPlace(Args&&... args) {
        ...
        if constexpr (can_handle<Message>::match) {
            ...
            for (auto i = 0u; i < entries.size(); ++i) {
                ...
                if (i == entries.size() - 1) {
                    e.transport->buffer
                        .template putInPlace<MessageWrap<M>>(IgnorePushedOut{}, std::forward<Args>(args)...);
                } else {
                    e.transport->buffer.template putInPlace<MessageWrap<M>>(IgnorePushedOut{}, args...);
                }
            }
        } else {
    ...
    /**
     * @brief best effort to send a message to via the BlockingContextRt
    ...
    /**
     * @brief send a range of messages via the BlockingContextRt 
            ...
            for (auto i = 0u; i < entries.size(); ++i) {
                ...
                if (e.deliverPred(*begin)) {
                    e.transport->buffer.template 
                        putBatchInPlace<MessageWrap<M>>(IgnorePushedOut{}, begin, n);
                }
            }
        } else {
...
private:
    void start(){}
    template <typename Message>
    using TransportEntry = blocking_context_rt_detail::TransportEntry<Message>;
            ...
            if (typeTagMatch<Message>(tag) 
                ...
                for (auto i = 0u; i < entries.size(); ++i) {
                    ...
                    if (e.deliverJustBytesPred(tag, (uint8_t const*)bytes)) {
                        ...
                        e.transport->buffer.template 
                            putInPlace<MessageWrap<JustBytes>>(IgnorePushedOut{}, tag, bytes, len, att);
                    }
                }
            }
        }
	...
	template <typename CcClient, typename RunOnceFunc>
    auto kickOffClientThread(RunOnceFunc runOnceFunc, CcClient& c, pattern::BlockingBufferRt* buffer
        ...
        std::thread thrd([
            ...
            if (stopped_) {
                if (buffer) { /// drain all to release sending party
                    bool more;
                    do {
                        uint8_t msgSpace[buffer->maxItemSize()];
                        more = buffer->tryTake(blocking_context_rt_detail::TypeSafeCopyOut<typename std::decay_t<CcClient>::Interests>{}
                            , msgSpace);
                    } while (more);
                }
                if (stillIn) {
            ...
            for (auto i = 0u; i < entries.size(); ++i) {
                ...
                if (e.transport->buffer.maxItemSize() >= sizeof(MessageWrap<M>)) {
                    if (e.deliverPred(begin->getTypeTag(), (uint8_t*)&*begin)) {
                        e.transport->buffer.template putBatchInPlace<MessageWrap<M>>(IgnorePushedOut{}, begin, n);
                    }
                } else {
                    HMBDC_THROW(std::out_of_range
                        , "need to increase maxItemSize in start() to >= "
